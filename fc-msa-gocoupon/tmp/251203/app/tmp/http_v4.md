...
```bash
curl -X POST http://localhost:8080/api/v4/coupons/issue \
  -H "Content-Type: application/json" \
  -H "X-USER-ID: USER_1" \
  -d '{
    "policy_code": "BF-C10"
...
```bash
curl -X POST http://localhost:8080/api/v4/coupons/use \
  -H "Content-Type: application/json" \
  -H "X-USER-ID: USER_1" \
  -d '{
    "coupon_code": "",
...
```bash
curl -X POST http://localhost:8080/api/v4/coupons/cancel \
  -H "Content-Type: application/json" \
  -H "X-USER-ID: USER_1" \
  -d '{
    "coupon_code": ""
...
```bash
curl -X GET http://localhost:8080/api/v4/coupons/417719c1-b95f-4d25-82b6-b168baa02dea \
  -H "Content-Type: application/json" \
  -H "X-USER-ID: USER_1" \
  -i
```
