...
import (
	...
	_ "example.com/pricing/cmd/server/docs"
...
func main() {
	configFlag := flag.String("config", "configs/pricing.yaml", "Path to config file")
	flag.Parse()
	...
	handler := pricing.NewHandler(store, client, logger)

	e := echo.New()
	e.HideBanner = false
	e.Use(middleware.Logger())
	e.Use(middleware.Recover())
	e.Validator = &pricing.CustomValidator{Validator: validator.New()}

	v1 := e.Group("/api/v1/pricings")
	{
		...
		v1.POST("/rules", handler.CreatePricingRule)
		v1.GET("/:id", handler.GetPricing)
	}
