...
func (s *store) FindProductByName(ctx context.Context, name string) (*Product, error) {
	...
	if err := s.db.WithContext(ctx).First(&product, "name = ?", name).Error; err != nil {
		s.log.Error().
			Err(err).
			Str("product_name", name).
			Msg("Failed to find product by name")
		return nil, err
	}
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.First(&product, "id = ?", productID).Error; err != nil {
			s.log.Error().
				Err(err).
				Str("product_id", productID).
				Msg("failed to find product for stock reversal")
			return err
		}

		if product.Stock < quantity {
			s.log.Warn().
				Int("current_stock", product.Stock).
		...
		if err := tx.Save(&product).Error; err != nil {
			s.log.Error().
				Err(err).
				Str("product_id", productID).
				Msg("failed to update product stock after reversal")
			return err
		}
		...
		s.log.Info().
			...
			Int("decreased_by", quantity).
			Msg("successfully reversed product stock")
	...
	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.First(&product, "id = ?", productID).Error; err != nil {
			s.log.Error().
				Err(err).
				Str("product_id", productID).
				Msg("failed to find product for stock release")
			return err
		}
		...
		if err := tx.Save(&product).Error; err != nil {
			s.log.Error().
				Err(err).
				Str("product_id", productID).
				Msg("failed to update product stock after release")
			return err
		}
		...
		s.log.Info().
			Str("product_id", productID).
			Int("new_stock", product.Stock).
			Int("increased_by", quantity).
			Msg("successfully released product stock")
		return nil
	})
}
