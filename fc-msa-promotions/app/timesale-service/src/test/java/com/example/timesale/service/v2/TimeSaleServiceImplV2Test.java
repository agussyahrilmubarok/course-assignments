...
class TimeSaleServiceImplV2Test {
    ...
    @InjectMocks
    private TimeSaleServiceImplV2 timeSaleServiceImplV2;
    ...
    void testCreate_whenValidRequest_shouldReturnSavedTimeSale() throws JsonProcessingException {
        ...
        TimeSale result = timeSaleServiceImplV2.create(request);

        assertNotNull(result);
        assertEquals(10L, result.getQuantity());
        verify(productRepository).save(any(Product.class));
        verify(timeSaleRepository).save(any(TimeSale.class));
        verify(redissonClient).getBucket("time-sale:" + savedTimeSale.getId());
    ...
    void testCreate_whenStartTimeAfterEndTime_shouldThrowException() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class,
                () -> timeSaleServiceImplV2.create(request)
        );
    ...
    void testFindById_whenExistsInCache_shouldReturnTimeSale() throws JsonProcessingException {
        ...
        TimeSale cachedTimeSale = new TimeSale();
        cachedTimeSale.setId(timeSaleId);

        String cachedJson = "{}";

        when(redissonClient.getBucket("time-sale:" + timeSaleId)).thenReturn(rBucket);
        when(rBucket.get()).thenReturn(cachedJson);
        when(objectMapper.readValue(cachedJson, TimeSale.class)).thenReturn(cachedTimeSale);

        TimeSale result = timeSaleServiceImplV2.findById(timeSaleId);

        assertNotNull(result);
        assertEquals(timeSaleId, result.getId());
        verify(timeSaleRepository, never()).findById(anyString()); // Because cache hit
    }

    @Test
    void testFindById_whenCacheMiss_shouldLoadFromDbAndSetCache() throws JsonProcessingException {
        String timeSaleId = "ts-123";
        TimeSale timeSaleFromDb = new TimeSale();
        timeSaleFromDb.setId(timeSaleId);

        when(redissonClient.getBucket("time-sale:" + timeSaleId)).thenReturn(rBucket);
        when(rBucket.get()).thenReturn(null); // cache miss
        when(timeSaleRepository.findById(timeSaleId)).thenReturn(Optional.of(timeSaleFromDb));
        when(objectMapper.writeValueAsString(timeSaleFromDb)).thenReturn("{}");
    ...
    void testFindById_whenNotFound_shouldThrowException() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class,
                () -> timeSaleServiceImplV2.findById(timeSaleId)
        );
    ...
    void testFindAllOngoing_whenCalled_shouldReturnPageOfTimeSales() {
        ...
        Page<TimeSale> result = timeSaleServiceImplV2.findAllOngoing(pageable);
    ...
    void testPurchase_whenValidRequest_shouldReturnUpdatedTimeSale() throws InterruptedException, JsonProcessingException {
        ...
        when(redissonClient.getBucket("time-sale:" + timeSaleId)).thenReturn(rBucket);
        when(rBucket.get()).thenReturn(null); // simulate cache miss, fallback to repo
        ...
        TimeSale result = timeSaleServiceImplV2.purchase(request, userId);
    ...
    void testPurchase_whenLockNotAcquired_shouldThrowException() throws InterruptedException {
        ...
        IllegalStateException exception = assertThrows(IllegalStateException.class,
                () -> timeSaleServiceImplV2.purchase(request, "user-123")
        );

        assertEquals("Could not acquire lock for time sale purchase.", exception.getMessage());
    ...
    void testPurchase_whenInterrupted_shouldThrowException() throws InterruptedException {
        ...
        assertEquals("Thread was interrupted while waiting for lock.", exception.getMessage());
        verify(rLock, never()).unlock();
    }

    @Test
    void testPurchase_whenNotEnoughStock_shouldThrowException() throws InterruptedException, JsonProcessingException {
        String userId = "user-1";
        String timeSaleId = "ts-1";

        TimeSaleDTO.PurchaseRequest request = TimeSaleDTO.PurchaseRequest.builder()
                .timeSaleId(timeSaleId)
                .quantity(20L) // More than available stock
                .build();
        ...
        IllegalStateException exception = assertThrows(IllegalStateException.class,
                () -> timeSaleServiceImplV2.purchase(request, userId)
        );
