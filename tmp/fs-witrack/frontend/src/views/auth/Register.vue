...
<template>
    <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div>
            ...
            <div class="mt-1 relative">
                <input v-model="form.fullName" type="text" id="fullName" name="fullName" placeholder="Your full name"
                    ...
                    :class="{ 'border-red-500 ring-red-500': error?.fullName }" />
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                    <i data-feather="user" class="w-4 h-4 text-gray-400"></i>
                </div>
                <p class="mt-1 text-xs text-red-500" v-if="error?.fullName">
                    {{ error.fullName }}
                </p>
            </div>
        </div>
    ...
    <div class="mt-6 text-center text-sm">
        ...
        <RouterLink to="/auth" class="font-medium text-blue-600 hover:text-blue-500">
            Sign in
        </RouterLink>
    </div>
</template>